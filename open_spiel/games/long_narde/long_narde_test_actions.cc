...
void ActionEncodingTest() {
  ...
  // White (15 on board, score 0), Black (15 on board, score 0)
  ...
  // White has 13 checkers on head, 1 at pos 14, 1 at pos 19 (total 15) -> score 0.
  ...
  // Same board as above (15 White, 15 Black) -> scores {0, 0}
  SetupBoardState(lnstate, kXPlayerId, modified_board);
...
void UndoRedoTest() {
  ...
  // Now both sides have 15 on-board, no one borne-off
  SetupBoardState(lnstate, current_player, mid_game_board);
...
void PassMoveBehaviorTest() {
  ...
  // Fill out to 15 blacks on-board
  no_moves_board[kOPlayerId][kBlackHeadPos] += 12;

  // Now White has 1 borne-off (14 on-board) and Black has 0 borne-off (15 on-board)
...
void VerifyDicePlayBehavior() {
  ...
  //StartTest: test-vsdpb-1
  {
    ...
    // Put the missing 13 whites on pt 4 (idx 3) and 14 blacks at their head:
    board[kXPlayerId][3] += 13;                   // now 15 whites total
    board[kOPlayerId][kBlackHeadPos] += 14;       // now 15 blacks total
    // Zero out scores to eliminate phantom borne-offs:
  ...
  //StartTest: test-vsdpb-2
  {
    ...
    // Distribute the 13 missing whites between pts 6 (idx 5) & 9 (idx 8):
    board[kXPlayerId][5] += 7;    // now 8 on pt 6
    board[kXPlayerId][8] += 6;    // now 7 on pt 9
    // Distribute the 13 missing blacks between pts 1 (idx 0) & 4 (idx 3):
    board[kOPlayerId][0] += 7;    // now 8 on pt 1
    board[kOPlayerId][3] += 6;    // now 7 on pt 4
    // Zero out scores:
    SetupBoardState(lnstate, kXPlayerId, board);
...
void DirectBearOffTest() {
  ...
  //StartTest: test-dbo-1w
  {
    ...
    // W has 2 on board -> score 13. B has 15 on board -> score 0
    SetupBoardState(lnstate, kXPlayerId, board);
  ...
  //StartTest: test-dbo-1b
  {
    ...
    // W has 15 on board -> score 0. B has 2 on board -> score 13.
...
void SingleCheckerBearOffTest() {
  ...
  //StartTest: test-scbo-1w
  {
    ...
    // W has 1 on board -> score 14. B has 0 on board -> score 0.
    ...
    // W has 0 on board -> score 15. B has 1 on board -> score 14.
...
void BearOffLastCheckerTest() {
  ...
  //StartTest: test-bolc-1w
  {
    ...
    board[kOPlayerId][11] = 15; // Irrelevant black checkers
    // W has 1 on board -> score 14. B has 15 on board -> score 0
    SetupBoardState(lnstate, kXPlayerId, board);
  ...
  //StartTest: test-bolc-1b
  {
    ...
    // W has 15 on board -> score 0. B has 1 on board -> score 14
    SetupBoardState(lnstate, kOPlayerId, board);
    SetupDice(lnstate, {4, 5, 0, 0});
