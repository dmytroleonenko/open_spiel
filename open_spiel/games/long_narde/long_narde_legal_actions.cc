...
std::vector<Action> LongNardeState::LegalActions() const {
  ...
  // *** ADDED DEBUG LOG ***
  std::cout << "[DEBUG LA Entry] Player=" << CurrentPlayer() << std::endl;
  // *** END ADDED DEBUG LOG ***
  ...
  // *** ADDED DEBUG LOG ***
  std::cout << "[DEBUG LA State Check] Player=" << CurrentPlayer()
            << ", DiceSize=" << dice_.size();
  if (dice_.size() >= 2) { // Check size before accessing elements
       std::cout << ", D0=" << DiceValue(0);
       std::cout << ", D1=" << DiceValue(1);
  }
  std::cout << std::endl;
  // *** END ADDED DEBUG LOG ***

  // ADDED: Log the filtered list before encoding
  if (kDebugging && CurrentPlayer() == 1 && dice_.size() >= 2 && DiceValue(0) == 1 && DiceValue(1) == 1) {
      std::cout << "[DEBUG LA FilteredList] Player=1 Dice={1,1}:\n";
  ...
  if (kDebugging && final_actions.size() != legal_moves.size()) {
      std::cout << "[DEBUG LA HigherDieRule Applied] Filtered actions from "
                << legal_moves.size() << " to " << final_actions.size() << std::endl;
  }
  return final_actions;

  // **** REMOVED: Return actions without higher die rule ****
  /*
  if (kDebugging) {
      std::cout << "[DEBUG LA Final Return] Returning " << legal_moves.size() << " actions (Higher die rule skipped)." << std::endl;
  }
  return legal_moves; // Return the directly encoded actions
  */
}
...
std::set<CheckerMove> LongNardeState::GenerateAllHalfMoves(int player, bool moved_from_head_this_sequence) const {
  ...
  if (kDebugging) {
      std::cout << "DEBUG GenerateAllHalfMoves called for player " << player
                << " | moved_from_head_this_seq=" << moved_from_head_this_sequence << std::endl;
  }

  // For each checker belonging to the player
  for (int pos = 0; pos < kNumPoints; ++pos) {
    if (board(player, pos) <= 0) continue; // Skip points with no checkers for this player

    if (kDebugging) {
        std::cout << "  Checking checker at pos=" << pos << std::endl;
    }

    // For each usable die slot (always 4 slots)
    for (int die_idx = 0; die_idx < dice_.size(); ++die_idx) {
        if (IsDieUsable(die_idx)) { // Check if the die slot itself is usable
            ...
            if (kDebugging) {
                std::cout << "    Trying die_idx=" << die_idx << " (value=" << die_value << ") -> to_pos=" << to_pos << std::endl;
            }
            ...
            // ADDED: Log validation result
            if (kDebugging && player == 1 && pos == 11 && die_value == 1) {
                std::cout << "[DEBUG GAHM POST-CHECK] Result of IsValidCheckerMove: " << (is_valid ? "true" : "false") << std::endl;
            }

            if (is_valid) {
              if (kDebugging) {
                  std::cout << "      VALID: inserting move {pos=" << pos << ", to=" << to_pos << ", die=" << die_value << "}" << std::endl;
              }
              half_moves.insert(current_move);
            } else {
              if (kDebugging) {
                  std::cout << "      INVALID move check failed." << std::endl;
              }
            }
        } else {
             if (kDebugging) {
                 std::cout << "    Skipping die_idx=" << die_idx << " (not usable)" << std::endl;
             }
        }
    }
  }
...
struct ExplorationState {
  ...
  ExplorationState(std::unique_ptr<LongNardeState> s,
    ...
    : state(std::move(s)), current_sequence(seq), move_applied(move), depth(d), moved_from_head_in_sequence(moved_head) {} // Use passed flags
};
...
int LongNardeState::IterativeLegalMoves(
  ...
  while (!exploration_stack.empty()) {
    ...
    std::cerr << "[DEBUG ILM Popped State] Depth: " << current_depth
              ...
              << " | Initial: [";
    for(int d : current_state->InitialDice()) { std::cerr << d << " "; }
    ...
    if (current_exploration.current_sequence.size() == 1) {
        ...
        if (current_state->CurrentPlayer() == kXPlayerId && dice_match &&
            first_move.pos == 8 && first_move.to_pos == 5 && first_move.die == 3) {
             ...
             for (size_t i = 0; i < initial_dice.size(); ++i) { // This loop should now work
    ...
    if (sequence_limit_hit || current_depth > kMaxIterationDepth) {
      ...
      if (!current_sequence.empty()) {
         ...
         if (kDebugging) {
             ...
             for(const auto& m : current_sequence) { ss_limit << " {" << m.pos << "," << m.to_pos << "," << m.die << "}"; }
    ...
    if (current_state->IsTerminal()) {
      if (!current_sequence.empty()) {
         ...
         if (kDebugging) {
             ...
             for(const auto& m : current_sequence) { ss_term << " {" << m.pos << "," << m.to_pos << "," << m.die << "}"; }
    ...
    if (kDebugging && current_state->CurrentPlayer() == 1) { // Log only for player 1 for relevance
        std::stringstream ss;
    ...
    if (half_moves.empty() || current_sequence.size() >= game_->MaxGameLength()) {
      if (!current_sequence.empty()) {
          ...
          if (kDebugging) {
              ...
              for(const auto& m : current_sequence) { ss_base << " {" << m.pos << "," << m.to_pos << "," << m.die << "}"; }
    ...
    for (int i = 0; i < moves_to_explore.size(); ++i) {
      ...
      // *** ADDED: Log state BEFORE potential clone/move ***
      if (next_sequence.size() == 1 &&
          next_sequence[0].pos == 7 && next_sequence[0].to_pos == 4 && next_sequence[0].die == 3) {
          std::cerr << "\n[DEBUG ILM Pre-Clone] State AFTER apply {7,4,3}:";
          std::cerr << "\n  Player: " << current_state->CurrentPlayer();
          std::cerr << "\n  Dice (Usable): " << current_state->DiceToString();
          const auto& initial_dice_pre = current_state->InitialDice();
          std::cerr << "\n  Initial Dice: [";
          for (size_t i = 0; i < initial_dice_pre.size(); ++i) {
             std::cerr << initial_dice_pre[i] << (i == initial_dice_pre.size() - 1 ? "" : ", ");
          }
          std::cerr << "]";
      }
      // *** END ADDED LOG ***
      ...
      if (current_state->IsTerminal()) {
          ...
          if (kDebugging) {
              ...
              for(const auto& m : next_sequence) { ss_term_post << " {" << m.pos << "," << m.to_pos << "," << m.die << "}"; }
        ...
        if (!is_last_move) {
            ...
            if (!exploration_stack.empty()) {
              ...
              std::cerr << "\n[DEBUG ILM After Push (Clone)] Top State - Player: " << top_state->CurrentPlayer()
                        ...
                        << " | Initial: [";
              for(int d : top_state->InitialDice()) { std::cerr << d << " "; }
              std::cerr << "]";
            }
            // *** End Log ***
     ...
     if (!found_move_in_iteration && !current_sequence.empty()) {
         // ADDED: Log sequence before adding due to no branches explored
         if (kDebugging) {
             std::stringstream ss_nobranch;
             ss_nobranch << "[DEBUG ILM ADD SEQ NOBRANCH] Depth=" << current_depth << ":";
             for(const auto& m : current_sequence) { ss_nobranch << " {" << m.pos << "," << m.to_pos << "," << m.die << "}"; }
...
std::vector<Action> LongNardeState::ApplyHigherDieRuleIfNeeded(
  ...
  // *** ADDED DEBUG LOG ***
  if (kDebugging) {
      std::cout << "[DEBUG AHDR Entry] Player=" << CurrentPlayer()
                << " Input actions (" << current_legal_moves.size() << ")\n";
      for(Action a : current_legal_moves) { std::cout << "  Action: " << a << std::endl; }
  }
  // *** END ADDED DEBUG LOG ***
  ...
  // *** ADDED DEBUG LOG ***
  if (kDebugging) {
      std::cout << "[DEBUG AHDR Stats] max_non_pass=" << max_non_pass
                << ", is_doubles=" << is_doubles << std::endl;
  }
  // *** END ADDED DEBUG LOG ***
  ...
  if (max_non_pass == 1 && !is_doubles) {
      // *** ADDED DEBUG LOG ***
      if (kDebugging) { std::cout << "[DEBUG AHDR Rule Applied] Rule condition met." << std::endl; }
      // *** END ADDED DEBUG LOG ***
      ...
      SPIEL_CHECK_GE(d1, 1); SPIEL_CHECK_LE(d1, 6);
      SPIEL_CHECK_GE(d2, 1); SPIEL_CHECK_LE(d2, 6);
      ...
      // *** ADDED DEBUG LOG ***
      if (kDebugging) {
          std::cout << "[DEBUG AHDR Found] found_higher=" << found_higher
                    << " (" << actions_using_higher.size() << " actions), found_lower=" << found_lower
                    << " (" << actions_using_lower.size() << " actions)" << std::endl;
      }
      // *** END ADDED DEBUG LOG ***
  ...
  // *** ADDED DEBUG LOG ***
  if (kDebugging && !(max_non_pass == 1 && !is_doubles)) {
      std::cout << "[DEBUG AHDR Rule Skipped] Rule condition NOT met. Returning original actions." << std::endl;
  }
  // *** END ADDED DEBUG LOG ***
