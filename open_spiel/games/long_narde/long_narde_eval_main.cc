...
int main(int argc, char** argv) {
  ...
  int report_every = std::max(1, config.report_every);
  for (int g = 0; g < config.games; ++g) {
    search_a.ClearCache();
    search_b.ClearCache();
    std::unique_ptr<open_spiel::State> state = game->NewInitialState();
    auto* lnstate = static_cast<open_spiel::long_narde::LongNardeState*>(
        state.get());
    ...
    while (!lnstate->IsTerminal() && moves < config.max_moves) {
      ...
      bool use_nnue = (player == kXPlayerId) ? use_a : use_b;
      Action action = kInvalidAction;
      if (use_nnue) {
        auto& search = (player == kXPlayerId) ? search_a : search_b;
        auto result = search.Search(lnstate);
    ...
    result.games += 1;
    if (config.progress &&
        ((g + 1) % report_every == 0 || g + 1 == config.games)) {
      double pct = 100.0 * static_cast<double>(g + 1) /
                   static_cast<double>(config.games);
      std::cout << "\r[eval] " << (g + 1) << "/" << config.games << " ("
