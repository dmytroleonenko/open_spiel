...
namespace open_spiel
{
  namespace long_narde
  {
    ...
    void LongNardeState::DoApplyAction(Action move_id)
    {
      ...
      // The incoming move_id is assumed to be a valid action generated by LegalActions().
      // Decode the Spiel action into its constituent checker moves.
      std::vector<LongNardeCheckerMove> checker_moves = LongNardeSpielMoveToCheckerMoves(cur_player_, move_id);

      // Apply each checker move.
      // LongNardeApplyCheckerMove will update moved_from_head_ if a head move occurs.
      for (const auto &m : checker_moves)
      {
        if (kDebugging) std::cerr << "[LN_DEBUG API]   DoApplyAction: Applying sub-move (Pos:" << m.pos << " To:" << m.to_pos << " Die:" << m.die << ")" << std::endl;
        LongNardeApplyCheckerMove(cur_player_, m);
        if (kDebugging) std::cerr << "[LN_DEBUG API]   DoApplyAction: After ApplyCheckerMove. MovedFromHead: " << moved_from_head_ << std::endl;
      }

      // Store turn history. `moved_from_head_` reflects its state after moves have been applied.
      ...
      LongNardeAdvanceToNextPlayer(checker_moves, move_id);
    }
    ...
    {
      if (kDebugging) {
        std::cerr << "[LN_DEBUG API] ProcessChanceRoll: START. MoveID: " << move_id
                  << ", PrevPlayer: " << prev_player_ << std::endl;
      }
      SPIEL_CHECK_TRUE(IsChanceNode());
      ...
      initial_dice_ = dice_;
      // Set first-phase-of-doubles flag if a doubles roll occurred.
      // A doubles roll means all 4 dice slots in initial_dice_ will be non-zero and equal.
      // So, checking initial_dice_[2] != 0 is a reliable way to detect a doubles roll.
      is_first_phase_of_doubles_ = (initial_dice_[2] != 0);
      moved_from_head_ = false; // Reset for the new player's turn.
    ...
    void LongNardeState::LongNardeAdvanceToNextPlayer(const std::vector<LongNardeCheckerMove>& checker_moves, Action move_id) {
      bool was_doubles_roll = (initial_dice_[0] == initial_dice_[1] && initial_dice_[0] != 0);
      ...
      next_player_id = NextPlayerRoundRobin(cur_player_, num_players_);
