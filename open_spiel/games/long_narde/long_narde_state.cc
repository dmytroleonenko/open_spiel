...
void LongNardeState::ApplyCheckerMove(int player, const CheckerMove& move) {
  ...
  if (move.pos == kPassPos) {
    ...
    for (int i = 0; i < dice_.size(); ++i) {
        if (dice_[i] == kPassDieValue) { // Find a '1'
  ...
  if (!found_die) {
      ...
      if (move.to_pos == kBearOffPos) {
         ...
         if (move.pos == furthest_pos) { // Was this move bearing off the furthest checker?
             for (int i = 0; i < dice_.size(); ++i) {
                 if (dice_[i] > 0 && dice_[i] >= move.die) { // Found a usable die >= the die value needed
                     dice_[i] = -1;
      ...
      if (!found_alternative) {
         ...
         std::cerr << "Initial Dice: { "; for(int d : initial_dice_) std::cerr << d << " "; std::cerr << "}\n";
