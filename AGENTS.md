...
- Inference is local-only. Use `inference.enable_local_batching=true` if you want to exercise the batching path; remote RPC inference was removed because mctx/JAX actors run under JIT.
...
- Quick launcher: `scripts/launch_local_muzero_services.sh --replay 1 --publisher 1 --log-dir /tmp` (no inference servers) starts replay/publisher and prints endpoints/log paths.
